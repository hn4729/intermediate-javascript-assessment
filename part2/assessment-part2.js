...
function noWeakLink() {
  return $http({
    ...
    .then(res => {
      return (firstUser = res.data[0]), res.data;
    })
    .then(res => {
      return (thirdUser = res[2]), res[9];
    });
}
...
function frodo(startingHungerValue, startingDangerValue) {
  ...
  return {
    dinnerOverFire: () => {
      hunger -= 25;
      danger += 40;
      hunger < 0 ? (hunger = 0) : hunger > 100 ? (hunger = 100) : null;
      danger < 0 ? (danger = 0) : danger > 100 ? (danger = 100) : null;
      return {
        hunger,
        danger
      };
